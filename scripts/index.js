...
const game = {
  ...
  fetchTheBacon: () => {
    ...
    if (game.currentMode === "easy" && game.currentRound === "1") {
      paragraph =
        game.text50[Math.floor(Math.random() * game.numberOfParagraph)];
      game.totalTime = 180;
      game.timeRemaining = 180;
    } else if (
      (game.currentMode === "easy" && game.currentRound === "2") ||
      (game.currentMode === "normal" && game.currentRound === "1")
    ) {
      paragraph =
        game.text70[Math.floor(Math.random() * game.numberOfParagraph)];
      game.totalTime = 165;
      game.timeRemaining = 165;
    } else if (
      (game.currentMode === "easy" && game.currentRound === "3") ||
      (game.currentMode === "normal" && game.currentRound === "2") ||
      (game.currentMode === "hard" && game.currentRound === "1")
    ) {
      paragraph =
        game.text100[Math.floor(Math.random() * game.numberOfParagraph)];
      game.totalTime = 150;
      game.timeRemaining = 150;
    } else if (
      (game.currentMode === "normal" && game.currentRound === "3") ||
      (game.currentMode === "hard" && game.currentRound === "2")
    ) {
      paragraph =
        game.text120[Math.floor(Math.random() * game.numberOfParagraph)];
      game.totalTime = 135;
      game.timeRemaining = 135;
    } else if (game.currentMode === "hard" && game.currentRound === "3") {
      paragraph =
        game.text150[Math.floor(Math.random() * game.numberOfParagraph)];
      game.totalTime = 120;
    ...
    if (paragraph && paragraph.length > 0) {
      ...
      $("#target").append(
        '<img id="end-typing-sign" src="images/dual-sword.png" alt="">'
      );
      $("#target").append("</p>");
  ...
  displayRound: function () {
    ...
    if (game.currentRound === "1") {
      roundDisplay.html('<img class="star" src="images/star.png" alt="star">');
  ...
  updateHeroGauge: function () {
    ...
    if (game.percentageCompletion >= 35 && game.percentageCompletion < 45) {
      ...
      game.percentageCompletion >= 45 &&
      game.percentageCompletion < 80
    ) {
  ...
  handleKeyup: function (event) {
    if (game.isRunning) {
      ...
      if (/^[a-z ,.'-]$/i.test(event.key)) {
        ...
        $("#target > span")
          .eq(pastCursorPosition)
          .removeClass("cursorPosition");
        $("#target > span")
          .eq(pastCursorPosition)
          .removeClass("cursorPosition-bkg");
      } else {
        return;
      }
  ...
  checkWordCompletion: function () {
    ...
    game.percentageCompletion = Math.floor(
      (game.completedWordsCount / game.totalWords) * 100
    );
    ...
    if (
      (game.numberIncorrectWords / game.totalWords) * 100 >
      game.incorrectWordsBreakPoint
    ) {
      game.loseGame();
    }

    if (
      game.percentageCompletion === game.percentageToLaunchFinalAttack &&
      ...
      game.timeoutId5 = setTimeout(function () {
        game.hpMonster.css("width", "0%");
      }, 1200);
    } else if (
      game.percentageCompletion >= game.percentageToLaunchFirstAttack &&
      game.runFirstAnimation === false
    ) {
      game.heroFirstAttackAnimation();
      //update monster HP bar after the attack
      game.timeoutId = setTimeout(function () {
        game.hpMonster.css("width", "60%");
      }, 1200);
  ...
  showIncorrectWords: function () {
    game.percentageIncorrectWords = Math.round(
      (game.numberIncorrectWords / game.completedWordsCount) * 100
    );
  ...
  heroFirstAttackAnimation: function () {
    if (game.chosenHero === "fire") {
      ...
      game.timeoutId = setTimeout(function () {
        game.heroFire.fadeIn();
      }, 1000);
      // Fireball animation
      $("#fireball")
        .addClass("fireball-animation")
        .show()
      ...
      game.timeoutId = setTimeout(function () {
        $("#water-gun-audio")[0].play();
      }, 500);
      game.timeoutId = setTimeout(function () {
        $("#water-spelling").removeClass("spelling").hide();
      }, 1000);
      game.timeoutId = setTimeout(function () {
        game.heroWater.fadeIn();
      }, 1000);
      // Water gun animation
      game.timeoutId = setTimeout(function () {
        $("#watergun")
          .addClass("hero-attack-1")
          .show()
          .one("animationend", function () {
            $("#watergun").removeClass("hero-attack-1").hide();
            game.runFirstAnimation = true;
      ...
      game.timeoutId = setTimeout(function () {
        $("#leaf-spelling").removeClass("spelling").hide();
      }, 1000);
      game.timeoutId = setTimeout(function () {
        game.heroLeaf.fadeIn();
      }, 1000);
  ...
  audioOff: function () {
    game.controlAudio.html(
      'OFF <img class="audio-icon" src="images/muted-audio.png" alt="muted audio icon">'
    );
  ...
  controlAudioEachScreen: function () {
    if (game.isAudioPlaying) {
      ...
      if (game.currentScreen === "splash-scr") {
        ...
        $("#battle-audio").trigger("play");
      } else if (game.currentScreen === "end-game-scr") {
        if (game.gameResult === "won") {
          // Unmute and play the win audio from the beginning
          $("#win-audio").prop("currentTime", 0);
          $("#win-audio").prop("muted", false);
          $("#win-audio").trigger("play");
        } else if (
          game.gameResult === "lost" ||
          game.gameResult === "unavailable" ||
          game.gameResult === "draw"
        ) {
          // Unmute and play the lose audio from the beginning
          $("#lose-audio").prop("currentTime", 0);
          $("#lose-audio").prop("muted", false);
          $("#lose-audio").trigger("play");
        }
      }
    } else {
      return;
  ...
  startGame: function () {
    if (
      ...
      $('[name="difficulty"]:checked').val() &&
      $('[name="hero"]:checked').val()
    ) {
  ...
  showGameResult: function () {
    ...
    switch (result) {
      case "unavailable":
        game.resultMessages.html(
          "<p><span>Unavailable</span><br> Sorry you did not enter the battle so there is nothing to report. Please go back and play...</p>"
        );
      ...
      case "draw":
        game.resultMessages.html(
          "<p><span>Draw</span><br> We will forever remember your sacrifice...</p>"
        );
        game.drawSign.show();
        $("#summary-box").show();
        $(".btn-next-round").hide();
      ...
      case "lost":
        game.resultMessages.html(
          "<p><span>You Lose!</span><br> Unfortunately, the monster is too strong. But, don't give up! We know you can do it!!!</p>"
        );
        $("#summary-box").show();
        $(".btn-next-round").hide();
        if (game.chosenHero === "fire") {
      ...
      case "won":
        game.resultMessages.html(
          "<p><span>You Win!</span><br> Congratulations...you are a true hero!!!</p>"
        );
        $("#summary-box").show();
        if (game.currentRound === "3") {
          // this condition prevents to show the Next Round button as round 3 is the maximum number of round in one mode.
          $(".btn-next-round").hide();
        } else {
          $(".btn-next-round").show();
        }
      ...
      default:
        game.resultMessages.html(
          "<span>Loading...</span> So sorry...the system gets errors to show the summary"
        );
    }
  },
  ...
  init: () => {
    ...
    game.startGame();
    $('#inputname, [name="difficulty"], [name="hero"]').on(
      "change",
      game.startGame
    );
      ...
      $('[name="difficulty"]').each(function () {
        if ($(this).val() === "hard") {
          ...
          $(this)
            ...
            .css(
              "background-color",
              game.selectedOption === "normal" ? "orange" : ""
            );
        } else {
    ...
    $(".btn-next-round").on("click", function () {
      ...
      switch (round) {
        ...
        default:
          alert(
            "Sorry...there is an error! Please go back to the Menu or try to refresh the page."
          );
      }
    });
